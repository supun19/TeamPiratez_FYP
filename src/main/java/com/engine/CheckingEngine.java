...
import com.markingschema.AnswerScript;
import com.markingschema.AnswerToGeoRelation;
import com.markingschema.MarkingSchema;
...
public class CheckingEngine {
	 ...
	 * algorithm
	 * 
	 * while there are elements in students answer if:element is present in
	 * inferred knowledge add element to studentAnswerKnowledgeHolder mark step
	 * as correct else run inference engine with loading
	 * studentAnswerKnowledgeHolder add new knowledge to inferred knowledge
	 * 
	 * if:element is present in inferred knowledge student has missed a step
	 * current step is correct else current step is wrong
	 */
	...
	public void check() throws Throwable {
			...
			if (inferedKnowledge.relationExists(step)) {
				...
				if (inferedKnowledge.relationExists(step)) {
					...
					if (inferedKnowledge.relationExists(step)) {
						...
						System.out.println(
								step.getName() + " step is present after inferencing using previously iferenced data");
		...
		markingSchemaHolder=new MarkingSchemaHolder();
		giveMarkForStudent();
		answerHolder.printAfterGiveMark();
	}
	...
	private void loadAnswer() {
		...
		String filename = "src\\main\\java\\com\\engine\\studentAnswer.txt"; // full
																													// correct
		// String filename = "E:\\eclipse\\data\\testdata2.txt"; // incorrect
		// step
	...
	private KnowledgeHolder loadInitialGeoRelationsDUMMY() {
		...
		/*
		 * Point A = new Point('A'); Point B = new Point('B'); Point C = new
		 * Point('C'); Point D = new Point('D'); Point E = new Point('E'); Point
		 * F = new Point('F');
		 * 
		 * Line AB = new Line(A, B); Line CD = new Line(C, D); Line EF = new
		 * Line(E, F); Line AE = new Line(A, E); Line EB = new Line(E, B); Line
		 * CF = new Line(C, F); Line FD = new Line(F, D);
		 * 
		 * Angle AEF = new Angle(EF,AE); Angle FEB = new Angle(EB,CD); Angle CFE
		 * = new Angle(CF, EF); Angle EFD = new Angle(EF, FD);
		 * 
		 * 
		 * GeoRelation ABparrCD = new GeoRelation(AB, CD,
		 * Relation.PARALLEL_LINES); GeoRelation AEparrCF = new GeoRelation(AE,
		 * CF, Relation.PARALLEL_LINES); // GeoRelation ABparrCD = new
		 * GeoRelation(AB, CD, Relation.PARALLEL_LINES); GeoRelation EonAB = new
		 * GeoRelation(AB, E, Relation.ON_THE_LINE); GeoRelation FonCD = new
		 * GeoRelation(CD, F, Relation.ON_THE_LINE);
		 * 
		 * ArrayList<GeoRelation> initRelations = new ArrayList<GeoRelation>();
		 * 
		 * initRelations.add(ABparrCD); initRelations.add(AEparrCF);
		 * 
		 * initRelations.add(EonAB); initRelations.add(FonCD);
		 * 
	...
	private void loadInitialGeoItamDataDUMMY() {
		 ...
		 * Point A = new Point('A'); Point B = new Point('B'); Point C = new
		 * Point('C'); Point D = new Point('D'); Point E = new Point('E'); Point
		 * F = new Point('F'); Point G = new Point('G');
		 * 
		 * Line AB = new Line(A, B); Line CD = new Line(C, D); Line EF = new
		 * Line(E, F); Line AE = new Line(A, E); Line EB = new Line(E, B); Line
		 * CF = new Line(C, F); Line FD = new Line(F, D);
		 ...
		 * 
		 * initData.add(A); initData.add(B); initData.add(C); initData.add(D);
		 * initData.add(E);
		 * 
		 * initData.add(AB); initData.add(CD); initData.add(EF);
		 * initData.add(AE); initData.add(EB); initData.add(CF);
		 * initData.add(FD);
		 * 
		 * initData.add(AEF); initData.add(FEB); initData.add(CFE);
		 * initData.add(EFD);
		 * 
	...
	private void giveMarkForStudent() {
		
		//List<GeoRelation> schemaSteps=answerToGeoRelation.getRelationStep(answerscript);
		System.out.println("marking student answer");
		//List<GeoRelation> markingschema=markingSchemaHolder.getSchemaRelation();
		answerHolder.initPostion();
		while (!answerHolder.endOfAnswer()) {
			GeoRelation step = answerHolder.getNextAnswerStep();
			if(answerHolder.getStatusFromCurrentRelation()==StepStatus.CORRECT){	
				markingSchemaHolder.giveMarkStep(step);
